...
def tab3_view(df, df19, mmr):
    ...
    <div class="blocked-text">
        ...
        The lighter purple bars represent 2019 measles cases, which was the previous biggest outbreak in the US before 2025, which are represented by the darker purple bars.
    </div>
    ...
    missing_2018 = filtered_mmr_comparison[y_2018.isna()]["code"].tolist()
    missing_2023 = filtered_mmr_comparison[y_2023.isna()]["code"].tolist()
    missing_both = set(missing_2018) | set(missing_2023)
    
    # State count below threshold in 2023–24
    below_thresh = (y_2023 < threshold).sum()
    
    fig = go.Figure()
    
    # Connecting lines between the two years for each state
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers',
        name="2018–19",
        marker=dict(size=10, color="#B497BD", symbol="circle"),
        hovertemplate="%{y}<br>2018–19: %{x:.1f}%" 
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers',
        name="2023–24",
        marker=dict(size=10, color="#6A4C93", symbol="circle-open"),
        hovertemplate="%{y}<br>2023–24: %{x:.1f}%"
    ))
    ...
    <br> <br>
    This chart connects each state’s MMR vaccination rate in 2018–2019 (solid dot) with its rate in 2023–2024 (open dot), showing direction and magnitude of change. A dashed line marks the 95% herd immunity threshold. States without data in either year will only show one dot.
    </div>
    <div style='font-size: 0.8rem;'>
